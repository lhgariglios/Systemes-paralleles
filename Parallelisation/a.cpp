...
int main(int argc, char* argv[])
{
    ...
    size_t local_ants = nb_ants / size;
    
    for (int i = 0; i < size; ++i) {
        sendcounts[i] = (i == size - 1) ? (nb_ants - i * local_ants) : local_ants;
        if (i > 0) displs[i] = displs[i-1] + sendcounts[i-1];
    }
    
    // Vetores para receber todas as formigas em rank 0
    std::vector<int> all_pos_x(nb_ants);
    std::vector<int> all_pos_y(nb_ants);
    
    // Renderer et fenêtre para processo 0
    Window* win = nullptr;
